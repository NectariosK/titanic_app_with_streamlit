...
def main():
	...
	with left:
		...
		embarked_radio = st.radio( "Embarked", list(embarked_d.keys()), index=2, format_func= lambda x: embarked_d[x] )

	with right:
		age_slider = st.slider("Age", value=1, min_value=1, max_value=100)
		sibsp_slider = st.slider("Siblings", min_value=0, max_value=10)
		parch_slider = st.slider("Children", min_value=0, max_value=10)
